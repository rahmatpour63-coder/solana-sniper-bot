...
def main(page: ft.Page):
                ...
                while True:
                    ...
                    if "params" in data:
                        ...
                        if any("initialize2" in log for log in logs):
                            ...
                            asyncio.create_task(execute_raydium_swap(wallet_keypair, "DetectedTokenAddress", float(buy_amount.value)))
        except asyncio.CancelledError:
            add_log("🛑 Robot sniper stopped safety.", is_info=True)
        except Exception as e:
    ...
    def toggle_bot(e):
        ...
        if bot_task is None or bot_task.done():
            if not private_key_input.value:
                ...
                return
            try:
                user_wallet = Keypair.from_string(private_key_input.value)
                add_log("Phantom wallet linked successfully.", is_success=True)
                btn_action.text = "توقف ربات"
                btn_action.bgcolor = ft.Colors.RED_600
                page.update()
                bot_task = asyncio.create_task(solana_scanner(user_wallet))
            except Exception:
                status_text.value = "خطا: فرمت کلید خصوصی فانتوم اشتباه است!"
                page.update()
        else:
            bot_task.cancel()
    ...
    page.add(
        ft.Container(
            content=ft.Column([
                ft.Row([
                    ft.Icon(ft.Icons.TARGET, color=ft.Colors.RED_600, size=28),
